...
[<Fact>]
let ``sanitizeEnvironment keeps NODE_ENV when set`` () =
    try
        System.Environment.SetEnvironmentVariable("NODE_ENV", "development")
        let psi = System.Diagnostics.ProcessStartInfo()
        CommandSafety.sanitizeEnvironment psi
        Assert.True(psi.Environment.ContainsKey "NODE_ENV")
        Assert.Equal("development", psi.Environment.["NODE_ENV"])
    finally
        System.Environment.SetEnvironmentVariable("NODE_ENV", null)

[<Fact>]
let ``sanitizeEnvironment keeps PWD when set`` () =
    try
        System.Environment.SetEnvironmentVariable("PWD", "/tmp")
        let psi = System.Diagnostics.ProcessStartInfo()
        CommandSafety.sanitizeEnvironment psi
        Assert.True(psi.Environment.ContainsKey "PWD")
        Assert.Equal("/tmp", psi.Environment.["PWD"])
    finally
...
[<Fact>]
let ``validateCommand blocks at command with word boundary`` () =
    let result = CommandSafety.validateCommand "at -f script.sh now + 1 min"

    match result with
    | Error msg -> Assert.Contains("dangerous", msg)
    | Ok _ -> failwith "Expected Error for at command"

[<Fact>]
let ``validateCommand allows cat and bat`` () =
    let result1 = CommandSafety.validateCommand "cat file.txt"
    let result2 = CommandSafety.validateCommand "bat file.txt"

    match result1, result2 with
    | Ok _, Ok _ -> ()
    | _ -> failwith "Expected Ok for cat and bat"

[<Fact>]
let ``validateCommand blocks python and ruby inline execution`` () =
    let pyResult =
        CommandSafety.validateCommand "python -c 'import os; os.system(\"ls\")'"

    let rbResult = CommandSafety.validateCommand "ruby -e 'puts \"hello\"'"

    match pyResult, rbResult with
    | Error msg1, Error msg2 ->
        Assert.Contains("dangerous", msg1)
        Assert.Contains("dangerous", msg2)
    | _ -> failwith "Expected Error for inline python and ruby execution"

[<Fact>]
let ``validateCommand allows cat chown.txt`` () =
    let result = CommandSafety.validateCommand "cat chown.txt"

    match result with
    | Ok _ -> ()
    | Error msg -> failwithf "Expected Ok for cat chown.txt, got Error: %s" msg

[<Fact>]
let ``validateCommand blocks echo chown`` () =
    let result = CommandSafety.validateCommand "echo chown"

    match result with
    | Error msg -> Assert.Contains("dangerous", msg)
    | Ok _ -> failwith "Expected Error for echo chown"
